...
function orderDateKey(timestamp?: Timestamp) {
  return timestamp
...
function orderTimeLabel(timestamp?: Timestamp) {
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const unsubscribeOrders = onSnapshot(
      ...
      (snapshot) => {
        setOrders(snapshot.docs.map((item) => ({ id: item.id, ...item.data() }) as DashboardOrder));
  ...
  const dataLoading = ordersLoading || reservationsLoading;

  return <AdminShell>
    <section className="border-b border-[#070a08]/10 pb-6">
      <p className="text-xs font-black uppercase tracking-[.18em] text-[#8b1e2b]">Operação em tempo real</p>
      <div className="mt-2 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h1 className="text-3xl font-black tracking-[-.04em]">Central da operação</h1>
          <p className="mt-2 max-w-2xl text-sm text-[#7b887d]">Priorize o que precisa de decisão agora e acompanhe o salão sem sair da fila.</p>
        </div>
        <div className="flex flex-wrap gap-2"><QuickLink href="/admin/pedidos" icon={ClipboardList} label="Abrir fila" /><QuickLink href="/admin/kds" icon={ChefHat} label="Cozinha" tone="dark" /></div>
      </div>
    </section>
...
function DashboardLoading() {
  return <div className="mt-6 space-y-7" aria-busy="true" aria-label="Atualizando a central da operação">
    <div className="grid gap-px overflow-hidden rounded-2xl border border-[#070a08]/10 bg-[#070a08]/10 sm:grid-cols-2 xl:grid-cols-4">
      {[1, 2, 3, 4].map((item) => <div key={item} className="h-36 animate-pulse bg-white p-5"><div className="h-9 w-9 rounded-xl bg-[#e8efe5]" /><div className="mt-5 h-3 w-28 rounded bg-[#e8efe5]" /><div className="mt-2 h-7 w-16 rounded bg-[#e8efe5]" /><div className="mt-2 h-3 w-40 rounded bg-[#e8efe5]" /></div>)}
    </div>
    <div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_320px]">
      <div className="h-80 animate-pulse rounded-[22px] bg-white" />
      <div className="h-80 animate-pulse rounded-[22px] bg-[#10261a]/80" />
    </div>
  </div>;
}
