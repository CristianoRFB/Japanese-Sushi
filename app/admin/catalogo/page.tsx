...
const catalogImageOptions = [
  { value: '/menu/sushi-salmao.webp', label: 'Sushi de salmão' },
  { value: '/menu/sushi-atum.webp', label: 'Sushi de atum' },
  { value: '/menu/sashimi-salmao.webp', label: 'Sashimi de salmão' },
  { value: '/menu/temaki-salmao.webp', label: 'Temaki de salmão' },
  { value: '/menu/combinado-teiko.webp', label: 'Combinado Teiko' },
  { value: '/menu/agua.webp', label: 'Água' },
];
...
export default function CatalogPage() {
  ...
  useEffect(() => {
    ...
    const markLoaded = () => {
      loadedSources += 1;
      if (loadedSources === 3) setLoading(false);
    };
    const stopProducts = onSnapshot(query(collection(db, 'products'), where('brandId', '==', TEIKO_BRAND_ID), orderBy('displayOrder')), (snap) => {
      setProducts(snap.docs.map((item) => ({ id: item.id, ...item.data() }) as Product));
      markLoaded();
    }, () => {
      setError('Não foi possível carregar os produtos agora.');
      markLoaded();
    });
    const stopCategories = onSnapshot(query(collection(db, 'categories'), where('brandId', '==', TEIKO_BRAND_ID), orderBy('displayOrder')), (snap) => {
    ...
    const stopGroups = onSnapshot(query(collection(db, 'modifierGroups'), where('brandId', '==', TEIKO_BRAND_ID), orderBy('displayOrder')), (snap) => {
  ...
  function openEditor(product: Product | null) {
    ...
    setImageUrlDraft(optimizedImageUrl(product?.imageUrl) ?? '');
    setError('');
    setShowForm(true);
  }
