      ...
      <div>
        <div className="overflow-hidden rounded-[28px] bg-[#173323] text-white">
          <div className="grid grid-cols-[1fr_120px] items-center gap-3 p-6 sm:grid-cols-[1fr_180px]">
            ...
            <img className="aspect-square w-full rounded-[22px] object-cover" src={optimizedImageUrl(product.imageUrl, '/brand/teiko-sushi-atmosphere.webp')} alt={`Foto de ${product.name}`} />
          </div>
        </div>

        <section className="mt-6">
          <div className="flex items-baseline justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[.16em] text-[#8b1e2b]">Passo 1</p><h2 className="mt-1 text-xl font-black">Escolha o tamanho</h2></div><span className="text-xs font-bold text-[#7b887d]">Obrigatório</span></div>
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">{product.sizes.filter((candidate) => candidate.active).sort((a, b) => a.displayOrder - b.displayOrder).map((candidate) => <button type="button" key={candidate.id} onClick={() => chooseSize(candidate.id)} className={`min-h-24 rounded-[22px] border-2 p-4 text-left transition ${sizeId === candidate.id ? 'border-[#b5232b] bg-[#e8efe5] shadow-[0_8px_20px_rgba(130,32,79,.12)]' : 'border-transparent bg-white shadow-sm'}`}><span className="block text-lg font-black">{candidate.label}</span><span className="mt-1 block text-sm font-bold text-[#b5232b]">{candidate.basePriceCents > 0 ? formatBRL(candidate.basePriceCents) : 'Preço a confirmar'}</span>{candidate.includedModifiersCount !== undefined && <span className="mt-2 block text-[11px] text-[#7b887d]">{candidate.includedModifiersCount} {candidate.includedModifiersCount === 1 ? 'item incluído' : 'itens incluídos'}</span>}</button>)}</div>
        </section>

        {groups.length > 0 && <section className="mt-9 border-t border-[#b5232b]/10 pt-7">
          <div className="flex items-end justify-between gap-4"><div><p className="text-xs font-extrabold uppercase tracking-[.16em] text-[#8b1e2b]">Passo {activeIndex + 2}</p><h2 className="mt-1 text-xl font-black">Crie sua combinação</h2><p className="mt-1 text-sm text-[#7b887d]">Avance pelas categorias e acompanhe o preço em tempo real.</p></div><strong className="shrink-0 text-sm text-[#b5232b]">{activeIndex + 1}/{groups.length}</strong></div>
          <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-[#d6ded4]"><div className="h-full rounded-full bg-[#b5232b] transition-all" style={{ width: `${((activeIndex + 1) / groups.length) * 100}%` }} /></div>
          <div className="teiko-scrollbar-none -mx-4 mt-4 flex snap-x gap-2 overflow-x-auto px-4 pb-2 sm:mx-0 sm:px-0" aria-label="Etapas de personalização">
            {groups.map((group, index) => { if (!group) return null; const total = groupTotal(group.id); const ready = total >= group.minSelections && (!group.required || total > 0); return <button type="button" key={group.id} onClick={() => { setActiveGroupId(group.id); setError(''); }} aria-current={activeGroupId === group.id ? 'step' : undefined} className={`flex shrink-0 snap-start items-center gap-2 rounded-full border px-3.5 py-2 text-xs font-black transition ${activeGroupId === group.id ? 'border-[#b5232b] bg-[#b5232b] text-white' : 'border-[#b5232b]/12 bg-white text-[#7b887d]'}`}><span className={`grid size-5 place-items-center rounded-full text-[10px] ${ready && total ? 'bg-[#d6e7bf] text-[#070a08]' : 'bg-current/10'}`}>{ready && total ? <Check className="size-3" /> : index + 1}</span>{group.name}{total > 0 && <span className="rounded-full bg-white/20 px-1.5 py-0.5">{total}</span>}</button>; })}
          </div>

          {activeGroup && <div className="mt-4 rounded-[26px] border border-[#b5232b]/10 bg-white p-4 shadow-sm sm:p-6">
            <div className="flex items-start justify-between gap-4"><div><h3 className="text-xl font-black">{activeGroup.name}</h3><p className="mt-1 text-sm text-[#7b887d]">{activeGroup.description || (activeGroup.required ? `Escolha de ${activeGroup.minSelections} a ${activeGroup.maxSelections}` : `Opcional • escolha até ${activeGroup.maxSelections}`)}{activeGroup.freeIncludedCount !== undefined ? ` • ${activeGroup.freeIncludedCount} incluído(s)` : ''}</p></div><span className={`shrink-0 rounded-full px-3 py-1 text-xs font-black ${groupTotal(activeGroup.id) >= activeGroup.minSelections && groupTotal(activeGroup.id) <= activeGroup.maxSelections ? 'bg-[#d6e7bf]/25 text-[#3a5b35]' : 'bg-[#e8efe5] text-[#b5232b]'}`}>{groupTotal(activeGroup.id)}/{activeGroup.maxSelections}</span></div>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">{activeGroup.modifierIds.map((modifierId) => modifierMap.get(modifierId)).filter(Boolean).sort((a, b) => (a?.displayOrder ?? 0) - (b?.displayOrder ?? 0)).map((modifier) => { if (!modifier) return null; const quantity = quantityFor(activeGroup.id, modifier.id); const max = Math.min(activeGroup.maxPerModifier ?? 99, modifier.maxQuantity ?? 99); const priceLabel = !modifier.available ? 'Indisponível hoje' : modifier.premium ? `Acréscimo de ${formatBRL(modifier.priceCents)}` : !modifier.priceCents ? 'Sem acréscimo' : activeGroup.pricingMode === 'individual' ? `+${formatBRL(modifier.priceCents)} por unidade` : `Dentro da cota; extra +${formatBRL(modifier.priceCents)}`; return <div key={modifier.id} className={`flex min-h-20 items-center gap-3 rounded-[20px] border p-3.5 ${quantity ? 'border-[#b5232b]/40 bg-[#e8efe5]' : 'border-[#b5232b]/8 bg-[#f3f0e8]'} ${!modifier.available ? 'opacity-55' : ''}`}><button type="button" disabled={!modifier.available} onClick={() => change(activeGroup.id, modifier.id, quantity ? -quantity : 1, activeGroup.maxSelections, max, activeGroup.allowDuplicate)} className="flex min-w-0 flex-1 items-center gap-3 text-left"><span className={`grid size-7 shrink-0 place-items-center rounded-full border-2 ${quantity ? 'border-[#b5232b] bg-[#b5232b] text-white' : 'border-[#d6ded4]'}`}>{quantity ? <Check className="size-4" /> : null}</span><span className="min-w-0"><strong className="block truncate text-sm">{modifier.name}</strong><span className="mt-0.5 block text-xs text-[#7b887d]">{priceLabel}</span></span></button>{activeGroup.allowDuplicate && modifier.available && <div className="flex items-center rounded-full bg-white p-1 shadow-sm"><button type="button" className="grid size-8 place-items-center rounded-full disabled:opacity-30" disabled={!quantity} onClick={() => change(activeGroup.id, modifier.id, -1, activeGroup.maxSelections, max, true)} aria-label={`Remover ${modifier.name}`}><Minus className="size-3.5" /></button><span className="w-5 text-center text-sm font-black">{quantity}</span><button type="button" className="grid size-8 place-items-center rounded-full bg-[#070a08] text-white disabled:opacity-30" disabled={quantity >= max || groupTotal(activeGroup.id) >= activeGroup.maxSelections} onClick={() => change(activeGroup.id, modifier.id, 1, activeGroup.maxSelections, max, true)} aria-label={`Adicionar ${modifier.name}`}><Plus className="size-3.5" /></button></div>}</div>; })}</div>
            <div className="mt-5 flex items-center justify-between border-t border-[#b5232b]/8 pt-4"><Button type="button" variant="outline" disabled={activeIndex === 0} onClick={() => setActiveGroupId(groups[activeIndex - 1]!.id)} className="rounded-full"><ChevronLeft className="size-4" /> Anterior</Button>{activeIndex < groups.length - 1 ? <Button type="button" onClick={() => setActiveGroupId(groups[activeIndex + 1]!.id)} className="rounded-full bg-[#070a08] text-white">Próxima categoria <ChevronRight className="size-4" /></Button> : <span className="flex items-center gap-1 text-xs font-black text-[#3a5b35]"><Check className="size-4" /> Última categoria</span>}</div>
          </div>}
        </section>}

        <section className="mt-9 border-t border-[#b5232b]/10 pt-7"><label htmlFor="notes" className="text-xl font-black">Alguma observação?</label><textarea id="notes" maxLength={300} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Ex.: colocar a calda separada" className="mt-3 min-h-24 w-full rounded-[20px] border border-[#b5232b]/15 bg-white p-4 text-sm outline-none focus:border-[#b5232b]" /><p className="mt-1 text-right text-xs text-[#7b887d]">{notes.length}/300</p></section>
      </div>

      <aside className="hidden lg:block"><div className="sticky top-26 rounded-[28px] bg-[#070a08] p-6 text-white shadow-[0_24px_50px_rgba(53,25,36,.18)]"><Sparkles className="size-6 text-[#d6e7bf]" /><h2 className="mt-4 text-2xl font-black">Sua combinação</h2><p className="mt-1 text-sm text-white/60">{size?.label ?? 'Escolha um tamanho'} • {itemCount} {itemCount === 1 ? 'item' : 'itens'}</p>{summaryGroups.length > 0 && <div className="mt-5 max-h-52 space-y-3 overflow-y-auto pr-1">{summaryGroups.map((group) => <div key={group.id}><strong className="block text-xs text-[#d6e7bf]">{group.name}</strong><span className="mt-0.5 block text-xs leading-relaxed text-white/70">{group.names.join(', ')}</span></div>)}</div>}<strong className="mt-6 block text-3xl font-black text-[#c7a773]">{priceAvailable ? formatBRL(preview) : 'Preço a confirmar'}</strong><span className="text-xs text-white/50">{priceAvailable ? 'Preço atualizado a cada escolha' : 'A unidade ainda não publicou este valor'}</span>{error && <p role="alert" className="mt-4 rounded-xl bg-[#e3262e]/15 p-3 text-sm text-[#ffe1e1]">{error}</p>}<Button onClick={submit} disabled={!priceAvailable} className="mt-5 h-12 w-full rounded-full bg-[#d6e7bf] font-black text-[#070a08] hover:bg-[#d6e7bf] disabled:bg-white/15 disabled:text-white/55">{editId ? 'Salvar alterações' : priceAvailable ? 'Adicionar ao carrinho' : 'Preço pendente'} <ChevronRight className="size-4" /></Button></div></aside>
    </div>

    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-[#b5232b]/10 bg-white/95 p-3 shadow-[0_-12px_35px_rgba(53,25,36,.1)] backdrop-blur-xl lg:hidden"><div className="mx-auto flex max-w-lg items-center gap-3"><div className="min-w-0 flex-1"><strong className="block truncate text-sm">{size?.label ?? 'Escolha um tamanho'} • {itemCount} itens</strong><span className="text-lg font-black text-[#b5232b]">{priceAvailable ? formatBRL(preview) : 'Preço a confirmar'}</span></div><Button onClick={submit} disabled={!priceAvailable} className="h-12 rounded-full bg-[#b5232b] px-5 font-black text-white disabled:bg-[#d6ded4] disabled:text-[#7b887d]">{editId ? 'Salvar' : priceAvailable ? 'Adicionar' : 'Indisponível'} <ChevronRight className="size-4" /></Button></div>{error && <p role="alert" className="mx-auto mt-2 max-w-lg text-xs font-bold text-[#e3262e]">{error}</p>}</div>
  </main>;
}
