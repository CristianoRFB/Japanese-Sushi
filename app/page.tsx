      ...
      <section className="bg-teiko-ink text-teiko-paper">
        <div className="mx-auto grid max-w-6xl divide-y divide-white/10 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
          ...
          <div className="flex items-start gap-3 px-4 py-6 sm:px-6">
            ...
            <div><span className="block text-[10px] font-black uppercase tracking-[.18em] text-teiko-gold">Atendimento</span><strong className="mt-1 block text-sm">{availability.acceptingOrders ? `Pedidos abertos${availability.closesAt ? ` até ${availability.closesAt}` : ''}` : `Hoje · ${formatNextOpening(availability.nextOpening)}`}</strong></div>
          </div>
          <div className="flex items-start gap-3 px-4 py-6 sm:px-6">
